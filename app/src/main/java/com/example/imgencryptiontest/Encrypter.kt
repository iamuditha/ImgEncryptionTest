...
object Encrypter {
    ...
    private val ALGO_SECRECT_KEY = "AES"
    ...
    fun encryptToFile (keyStr: String, spec:String, input:InputStream, output: OutputStream)
    {
        var output = output
        try {
            var iv = IvParameterSpec(spec.toByteArray(charset("UTF-8")))
            val keySpec = SecretKeySpec(keyStr.toByteArray(charset("UTF-8")), ALGO_SECRECT_KEY)
